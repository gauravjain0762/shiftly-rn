...
const InterviewStatus = () => {
  ...
  const candidateName =
    candidateData?.name || inviteData?.user_id?.name || 'N/A';
  const desiredJobTitle =
    candidateData?.desired_job_title ||
    inviteData?.user_id?.desired_job_title ||
    '';
    console.log(candidateData,'candidateData?.responsibility',inviteData);
  ...
  const summaryData = inviteData?.interview_response?.summary || '';
  const transcriptData =
    inviteData?.interview_response?.transcript_with_timestamp || [];
  ...
  const [activeTab, setActiveTab] = React.useState<'General' | 'Languages'>(
    'General',
  );
  ...
  return (
    <LinearContainer colors={[colors.white, colors.white]}>
      ...
      <ScrollView
          ...
          <View style={styles.row}>
            ...
            <View style={styles.jobInfo}>
              ...
              <View style={styles.jobMetaRow}>
                <Text
                  style={styles.jobMeta}>{`${location} - ${contract}`}</Text>
              </View>
              {renderSalary()}
            </View>
          </View>
        ...
        <View style={[styles.card, {backgroundColor: '#BEDEFF3B'}]}>
          <View style={[styles.row, {alignItems: 'center'}]}>
            ...
            <View style={styles.candidateInfo}>
              ...
              {!!desiredJobTitle && (
                <Text style={styles.candidateDesiredTitle}>
                  {desiredJobTitle}
                </Text>
              )}
            </View>
            <View style={styles.statusCol}>
              <View
                style={[
                  styles.statusBadge,
                  !isInterviewCompleted && {backgroundColor: '#FFA500'},
                ]}>
                <Text style={styles.statusText}>{t(interviewStatus)}</Text>
              </View>
              {!!interviewDate && (
                <Text style={styles.dateText}>{interviewDate}</Text>
              )}
            </View>
          </View>
        </View>
        ...
        <View style={styles.mediaRow}>
          <TouchableOpacity
            ...
            <Image
              ...
              style={[
                styles.mediaIcon,
                !isInterviewCompleted && {tintColor: '#D3D3D3'},
              ]}
            />
            <Text
              style={[
                styles.mediaText,
                !isInterviewCompleted && {color: '#D3D3D3'},
              ]}>
              {t('Listen Audio')}
            </Text>
          </TouchableOpacity>
          <TouchableOpacity
            ...
            onPress={() => {
              if (interviewVideoUrl) {
                navigateTo(SCREENS.WebviewScreen, {
                  link: interviewVideoUrl,
                  title: t('Watch Interview'),
                });
              }
            }}>
            <Image
        ...
        {summaryData && (
          <>
            <Text style={styles.sectionTitle}>{t('Interview Summary')}</Text>
            <View style={styles.transcriptBox}>
              <Text style={[styles.transcriptText]}>{summaryData}</Text>
            </View>
          </>
        )}
              ...
              transcriptData.map((item: any, index: number) => (
                <Text
                  ...
                  style={[
                    ...
                    index > 0 && {marginTop: hp(12)},
                  ]}>
                  <Text style={styles.speaker}>
                    {item.role === 'agent' ? t('Agent') : t('User')}:{' '}
                  </Text>
                  {item.content}
                </Text>
              ))
        ...
        <TouchableOpacity
          ...
          <View>
            ...
            <Text style={styles.chatSubtitle}>{t('Start a conversation')}</Text>
          </View>
        </TouchableOpacity>
